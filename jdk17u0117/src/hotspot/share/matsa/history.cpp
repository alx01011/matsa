...
void History::add_event(JavaThread *thread, Method *m, uint16_t bci) {
    ...
    if (h->event_idx == MAX_EVENTS - 1) {
        ...
        memcpy(h->buffer[buffer_idx].real_stack, stack->_stack, stack->_top * sizeof(uint64_t));
        buffer[buffer_idx].real_stack_size = stack->_top;
    }

    EventBuffer *current_buffer = &h->buffer[buffer_idx];
    EventBuffer::Event *event = &current_buffer->events[event_idx];
