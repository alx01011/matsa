...
History::History() {
    ...
    for (int i = 0; i < MAX_PARTS; i++) {
        buffer[i].events = (Event*)os::reserve_memory(MAX_EVENTS * sizeof(Event));
        protect = os::protect_memory((char*)buffer[i].events, MAX_EVENTS * sizeof(Event), os::MEM_PROT_RW);
...
void History::add_event(JavaThread *thread, Method *m, uint16_t bci) {
    ...
    if (h->event_idx == MAX_EVENTS - 1) {
        ...
        memcpy(h->parts[h->part_idx].real_stack, stack->get(), stack->size() * sizeof(uint64_t));
        h->parts[h->part_idx].real_stack_size = stack->size();
    }

    Part *current_part = &h->parts[h->part_idx];
    Event *event = &current_part->events[current_part->event_idx];

    event->method = m;
    event->bci = bci;

    current_part->event_idx++;
}

Part *History::get_part(uint64_t tid, uint64_t idx) {
    assert(idx < MAX_BUFFERS, "MATSA: Invalid buffer index\n");
    return &history[tid]->part_idx[idx];
}
...
uint64_t History::get_event_idx(uint64_t tid) {
    return history[tid]->parts[history[tid]->part_idx].event_idx;
}
